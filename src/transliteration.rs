...
pub fn transliteration_similarity(
    ...
    let mut left = vec![a.raw.as_str()];
    left.extend(views_a);
    let mut right = vec![b.raw.as_str()];
    ...
    for one in &left {
        for other in &right {
            best = best.max(crate::lexical::character::combined_character_similarity(
                one, other,
            ));
        }
    }

...
use crate::core::config::{EngineConfig, SimilarityWeights};
...
impl TextIntelligence {
    ...
    pub fn try_new(config: EngineConfig) -> Result<Self, TextIntelError> {
        config.validate().map_err(TextIntelError::InvalidConfiguration)?;
        Ok(Self {
            config,
    ...
    pub fn with_language_provider<P: LanguageDetectionProvider + 'static>(mut self, provider: P) -> Self {
        self.language_provider = Arc::new(provider);
        self
    }

    pub fn with_lemmatizer_provider<P: LemmatizerProvider + 'static>(mut self, provider: P) -> Self {
        self.lemmatizer_provider = Some(Arc::new(provider));
        self
    }

    pub fn with_symbol_knowledge_provider<P: SymbolKnowledgeProvider + 'static>(mut self, provider: P) -> Self {
    ...
    fn check_length(&self, text: &str) -> Result<(), TextIntelError> {
        ...
        if length > self.config.max_input_length {
            return Err(TextIntelError::InputTooLong { length, maximum: self.config.max_input_length });
        }
        Ok(())
    }

    fn detect(&self, text: &str) -> Result<Vec<crate::core::types::LanguageCandidate>, TextIntelError> {
        self.language_provider.detect(text).map_err(TextIntelError::from)
    }
    ...
    fn build_phonetic_candidates(
        ...
        let language_list: Vec<String> = languages
            ...
            .collect();
        let language_list = if language_list.is_empty() { vec!["und".to_string()] } else { language_list };
        for language in &language_list {
            let candidate = self.g2p_provider.phonemize(raw, language).map_err(TextIntelError::from)?;
            if seen.insert((candidate.source.clone(), candidate.language.clone())) {
                values.push(candidate);
            }
            for decoded in rebus.iter().take(3) {
                let candidate = self.g2p_provider.phonemize(&decoded.text, language).map_err(TextIntelError::from)?;
                if seen.insert((candidate.source.clone(), candidate.language.clone())) {
                    values.push(candidate);
                }
            }
    ...
    pub fn analyze(&self, text: &str) -> Result<MessageFingerprint, TextIntelError> {
        ...
        let lemmas = match &self.lemmatizer_provider {
            Some(provider) => provider.lemmatize(&tokens, None).map_err(TextIntelError::from)?,
        ...
        let symbols = resolve_symbols_with_provider(text, self.config.max_symbol_readings, self.symbol_provider.as_ref());
        ...
        let normalized = normalize_whitespace(&collapse_repetition(&apply_leet(&casefold_text(&nfkc(text))), 1));
        let language_names: Vec<String> = languages.iter().map(|candidate| candidate.language.clone()).collect();
        let decoder = RebusDecoder::new(self.config.clone());
        let rebus = decoder.decode_with_provider(text, Some(&language_names), Some(self.config.max_candidates), self.symbol_provider.as_ref());
        ...
        if self.config.semantic {
            let values = self.embedding_provider.embed(&[text.to_string()]).map_err(TextIntelError::from)?;
            if values.len() > 1 {
                return Err(TextIntelError::Provider(ProviderError::new(
                    "embedding",
                    format!("returned {} vectors for one input", values.len()),
                )));
            }
            if let Some(vector) = values.into_iter().next() {
                if !vector.is_empty() { semantic_embeddings.insert("default".to_string(), vector); }
            }
        }
        let phonetic_candidates = self.build_phonetic_candidates(text, &languages, &rebus)?;
        let mut metadata = BTreeMap::new();
        metadata.insert("semantic_enabled".to_string(), self.config.semantic.to_string());
        metadata.insert("phonetic_enabled".to_string(), self.config.phonetic.to_string());
    ...
    pub fn decode_with_languages(
        ...
        self.check_length(text)?;
        let decoder = RebusDecoder::new(self.config.clone());
        Ok(decoder.decode_with_provider(text, languages, max_candidates, self.symbol_provider.as_ref()))
    }
    ...
    pub fn compare(&self, left: &str, right: &str) -> Result<ComparisonResult, TextIntelError> {
        ...
        Ok(score_fingerprints(&left, &right, &self.config.similarity_weights))
    }
    ...
    pub fn add_pattern(&self, id: impl Into<String>, examples: Vec<String>) -> Result<(), TextIntelError> {
        let id = id.into();
        if id.trim().is_empty() || examples.is_empty() {
            return Err(TextIntelError::InvalidConfiguration("pattern id and examples are required".to_string()));
        }
        let mut analyzed = Vec::with_capacity(examples.len());
        for example in &examples {
            analyzed.push((example.clone(), self.analyze(example)?));
        }
        let pattern = RegisteredPattern { pattern: Pattern { id: id.clone(), examples }, examples: analyzed };
        self.patterns
            .write()
    ...
    pub fn remove_pattern(&self, id: &str) -> Result<bool, TextIntelError> {
        Ok(self.patterns.write().map_err(|_| TextIntelError::Storage("pattern lock poisoned".to_string()))?.remove(id).is_some())
    }

    pub fn match_patterns(&self, text: &str) -> Result<Vec<PatternMatch>, TextIntelError> {
        let query = self.analyze(text)?;
        let patterns = self.patterns.read().map_err(|_| TextIntelError::Storage("pattern lock poisoned".to_string()))?;
        let mut matches = patterns
            .values()
            .filter_map(|registered| match_pattern_fingerprint(&query, &registered.pattern, &registered.examples, &self.config.similarity_weights))
            .collect::<Vec<_>>();
        matches.sort_by(|left, right| right.score.total_cmp(&left.score));
        Ok(matches)
    }

    pub fn detect_spam(&self, text: &str) -> Result<crate::core::types::SpamResult, TextIntelError> {
        let fingerprint = self.analyze(text)?;
        let patterns = self.match_patterns(text)?;
        Ok(predict_spam(&fingerprint, &patterns))
    }

    pub fn duplicate(&self, left: &str, right: &str, threshold: f64) -> Result<DuplicateResult, TextIntelError> {
        let left = self.analyze(left)?;
        let right = self.analyze(right)?;
        Ok(duplicate_result(&left, &right, threshold.clamp(0.0, 1.0), &self.config.similarity_weights))
    }

    pub fn add_document(&self, id: impl Into<String>, text: &str) -> Result<(), TextIntelError> {
        let id = id.into();
        if id.trim().is_empty() {
            return Err(TextIntelError::InvalidConfiguration("document id cannot be empty".to_string()));
        }
        let fingerprint = self.analyze(text)?;
        let mut store = self.store.write().map_err(|_| TextIntelError::Storage("store lock poisoned".to_string()))?;
        if store.len() >= self.config.max_documents && store.records().iter().all(|(current, _)| current != &id) {
            return Err(TextIntelError::Storage(format!("max_documents={} reached", self.config.max_documents)));
        }
        store.upsert(id, fingerprint).map_err(TextIntelError::Storage)
    }

    pub fn remove_document(&self, id: &str) -> Result<bool, TextIntelError> {
        self.store.write().map_err(|_| TextIntelError::Storage("store lock poisoned".to_string()))?.remove(id).map_err(TextIntelError::Storage)
    }

    pub fn document_count(&self) -> Result<usize, TextIntelError> {
        Ok(self.store.read().map_err(|_| TextIntelError::Storage("store lock poisoned".to_string()))?.len())
    }

    pub fn find_similar(&self, text: &str, limit: usize) -> Result<Vec<SearchResult>, TextIntelError> {
        if limit == 0 { return Ok(Vec::new()); }
        let query = self.analyze(text)?;
        let records = self.store.read().map_err(|_| TextIntelError::Storage("store lock poisoned".to_string()))?.records();
        let mut candidates = records
            .into_iter()
            .map(|(id, fingerprint)| {
                let comparison = score_fingerprints(&query, &fingerprint, &self.config.similarity_weights);
        ...
        let results = if let Some(reranker) = &self.reranker_provider {
            let input = candidates
                .iter()
                .map(|(id, fingerprint, comparison)| (id.clone(), fingerprint.clone(), comparison.score))
                .collect();
            let reranked = reranker.rerank(&query, input).map_err(TextIntelError::from)?;
            ...
            let mut reranked_results = reranked
                .into_iter()
                .filter_map(|(id, _, score)| by_id.remove(&id).map(|mut comparison| {
            ...
            candidates.into_iter().map(|(id, _, comparison)| (id, comparison)).collect()
        };
        Ok(results
            .into_iter()
            .take(limit)
            .map(|(id, comparison)| SearchResult { id, score: comparison.score, comparison })
            .collect())
    }

    pub fn find_duplicates(&self, text: &str, threshold: f64) -> Result<Vec<(String, DuplicateResult)>, TextIntelError> {
        let query = self.analyze(text)?;
        let records = self.store.read().map_err(|_| TextIntelError::Storage("store lock poisoned".to_string()))?.records();
        Ok(records.into_iter().map(|(id, fingerprint)| (id, duplicate_result(&query, &fingerprint, threshold.clamp(0.0, 1.0), &self.config.similarity_weights))).filter(|(_, result)| result.duplicate).collect())
    }
}

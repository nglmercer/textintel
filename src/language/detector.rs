...
const ES: &[&str] = &[
    ...
    "con", "para", "compra", "comprar", "ahora", "gana", "ganar", "dinero", "saludos",
    "saludo", "casa", "hogar", "fracasado", "hola", "gracias", "si", "sí", "siempre",
    "premio", "reclama", "tu", "dos", "fuego", "amor", "plata", "vivienda",
];
const EN: &[&str] = &[
    "the", "and", "of", "to", "a", "in", "is", "you", "that", "it", "for", "on", "with",
    "bro", "now", "buy", "money", "house", "home", "hi", "hello", "fire", "hot", "lit",
    "always", "love", "cash", "please", "claim", "your", "prize",
];
const PT: &[&str] = &[
    "o", "a", "os", "as", "de", "que", "e", "do", "da", "para", "com", "não", "dinheiro",
    "casa", "agora", "comprar",
];
const FR: &[&str] = &[
    "le", "la", "les", "de", "des", "et", "un", "une", "est", "pour", "avec", "bonjour",
    "maison", "argent", "acheter", "maintenant",
];

fn word_set(words: &[&str], word: &str) -> bool {
    words.iter().any(|candidate| *candidate == word)
}
...
pub fn detect_languages(text: &str) -> Vec<LanguageCandidate> {
    ...
    for word in &words {
        if word_set(ES, word) { *scores.entry("es".to_string()).or_default() += 2.0; }
        if word_set(EN, word) { *scores.entry("en".to_string()).or_default() += 2.0; }
        if word_set(PT, word) { *scores.entry("pt".to_string()).or_default() += 1.0; }
        if word_set(FR, word) { *scores.entry("fr".to_string()).or_default() += 1.5; }
    }
    for ch in text.chars().flat_map(char::to_lowercase) {
        if "áéíóúñü¿¡".contains(ch) { *scores.entry("es".to_string()).or_default() += 1.0; }
        if "ãõç".contains(ch) { *scores.entry("pt".to_string()).or_default() += 1.0; }
        if "àâæçéèêëîïôœùûüÿ".contains(ch) { *scores.entry("fr".to_string()).or_default() += 0.6; }
    }
    if scores.is_empty() {
        if words.is_empty() {
    ...
    let known_mass: f64 = candidates.iter().map(|candidate| candidate.probability).sum();
    ...
    let sum: f64 = candidates.iter().map(|candidate| candidate.probability).sum();

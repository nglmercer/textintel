...
fn fallback_leet_readings(token: &str, max_readings: usize) -> Vec<SymbolReading> {
    if token.chars().count() != 1 || !leet_map().contains_key(&token.chars().next().unwrap_or_default()) {
        return Vec::new();
    }
    leet_map()
...
pub fn token_readings_with_provider(
    ...
    if token.chars().any(char::is_numeric) && token.chars().any(char::is_alphabetic) {
        ...
        vec![
            (token.to_string(), 0.40, "und".to_string(), "identity".to_string()),
            (folded.clone(), 0.75, "und".to_string(), "leetspeak".to_string()),
            (folded.to_lowercase(), 0.80, "und".to_string(), "leetspeak".to_string()),
        ]
    } else {
        vec![
            (token.to_string(), 1.0, "und".to_string(), "identity".to_string()),
            (token.to_lowercase(), 0.95, "und".to_string(), "casefold".to_string()),
        ]
    }
}

...
fn known(text: &str) -> bool {
    WORDLIST.iter().any(|word| *word == text)
}

fn can_split_known(text: &str, depth: usize) -> bool {
    if depth > 4 || text.is_empty() {
        return false;
    }
    if known(text) {
        return true;
    ...
    (2..text.len().saturating_sub(1)).any(|index| {
        text.is_char_boundary(index)
            && known(&text[..index])
            && can_split_known(&text[index..], depth + 1)
    })
}
...
pub fn lexical_plausibility(text: &str) -> f64 {
    ...
    if known(&compact) || can_split_known(&compact, 0) {
        return 1.0;
    }
    ...
    if WORDLIST.iter().any(|word| word.starts_with(&compact) || compact.starts_with(word)) {
        return 0.55;
    }
    0.15
}
...
pub fn score_candidate(surface: &str, prior: f64) -> (f64, f64, f64, f64) {
    let lexical = lexical_plausibility(surface);

...
fn feature_distance(left: &str, right: &str) -> f64 {
    ...
    if vowels(left) == vowels(right) && voiced(left) == voiced(right) { 0.35 } else { 1.0 }
}
...
pub fn weighted_phoneme_distance(a: &[String], b: &[String]) -> f64 {
    ...
    for (i, left) in a.iter().enumerate() {
        ...
        for (j, right) in b.iter().enumerate() {
            ...
            current.push((current[j] + 1.0).min(previous[j + 1] + 1.0).min(substitute));
        }
        previous = current;
    }
    previous[b.len()]
...
pub fn phoneme_ngram_similarity(a: &[String], b: &[String], n: usize) -> f64 {
    ...
    let left = grams(a);
    let right = grams(b);
    if left.is_empty() && right.is_empty() { return if a == b { 1.0 } else { 0.0 }; }
    let keys: std::collections::BTreeSet<_> = left.keys().chain(right.keys()).collect();
    let intersection: usize = keys.iter().map(|key| left.get(*key).unwrap_or(&0).min(right.get(*key).unwrap_or(&0))).sum();
    let union: usize = keys.iter().map(|key| left.get(*key).unwrap_or(&0).max(right.get(*key).unwrap_or(&0))).sum();
    if union == 0 { 0.0 } else { intersection as f64 / union as f64 }
}

pub fn phonetic_similarity(a: &PhoneticCandidate, b: &PhoneticCandidate) -> f64 {
    if a.phonemes.is_empty() || b.phonemes.is_empty() { return 0.0; }
    ...
    (0.45 * edit + 0.25 * exact + 0.20 * grams + 0.10 * a.confidence.min(b.confidence)).clamp(0.0, 1.0)
}

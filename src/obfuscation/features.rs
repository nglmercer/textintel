...
fn uppercase_ratio(text: &str) -> f64 {
    ...
    if letters == 0 { 0.0 } else { uppercase as f64 / letters as f64 }
}
...
pub fn obfuscation_features(text: &str, unicode: &UnicodeFeatures) -> ObfuscationFeatures {
    ...
    let emoji_count = text.chars().filter(|ch| {
        let code = *ch as u32;
    ...
    let symbol_substitution_score = if emoji_count > 0 && letters > 0 { 0.35 } else { 0.0 };
    let leet_score = if leetspeak {
        (digits as f64 / letters.max(1) as f64).min(1.0).max(0.35)
    } else { 0.0 };
    let homoglyph_score = if confusables { (unicode.confusable_characters.len() as f64 * 0.35).min(1.0) } else { 0.0 };
    let unicode_score = unicode.suspicious_unicode_score;

    let mut flags = Vec::new();
    if leetspeak { flags.push("leetspeak".to_string()); }
    if repetition { flags.push("repetition".to_string()); }
    if flood { flags.push("punctuation_flood".to_string()); }
    if mixed_scripts { flags.push("mixed_scripts".to_string()); }
    if confusables { flags.push("confusables".to_string()); }
    if !unicode.invisible_characters.is_empty() { flags.push("invisible_characters".to_string()); }
    if !unicode.bidirectional_controls.is_empty() { flags.push("bidirectional_controls".to_string()); }
    if fragmentation_score >= 0.2 { flags.push("fragmentation".to_string()); }
    if uppercase_ratio(text) > 0.9 && letters >= 4 { flags.push("uppercase_flood".to_string()); }
